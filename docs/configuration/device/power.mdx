...
:::info
Power settings are advanced configuration, most users should choose a role under [Device Config](/docs/configuration/device/device) to manage power for their device and shouldn't ever need to adjust these settings.
:::
