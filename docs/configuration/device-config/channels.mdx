...
:::info
**Channel Settings** (as described on this page) should not be confused with [Modem Preset Settings](/docs/configuration/config/lora#modem-preset)

[Modem Preset Settings](/docs/configuration/config/lora#modem-preset) contain the modem configuration (frequency settings, spreading factor, bandwidth, etc.) used for the LoRa radio. These settings are identical for all channels and can **not** be unique per channel.
...
For configuring gateways, please see [MQTT](/docs/configuration/moduleconfig/mqtt)
