...
Telemetry is shared over your [PRIMARY channel](/docs/configuration/config/channels#role).  This means that if your node has acquired GPS coordinates from an integrated GPS chip, or from your mobile device, your coordinates will be sent to the mesh over this channel, using it's defined encryption (if any).
...
1. Ensure you have not changed the LoRa [Modem Preset](/docs/configuration/config/lora#modem-preset) from the default `unset` / `LONG_FAST`.
...
Meshtastic nodes will rebroadcast all packets if they share [LoRa modem settings](/docs/configuration/config/lora#lora-config-values), regardless of encryption (unless [Rebroadcast mode](/docs/configuration/config/device#rebroadcast-mode) is set to `LOCAL_ONLY`).

:::info
 If you would like your nodes to include/expand the "public" mesh, you must use the default [modem preset](/docs/configuration/config/lora#modem-preset) `LONG_FAST`.  If you change your PRIMARY channel name, you must manually set the LoRa channel to 20.
:::

## Chat Channels VS LoRa Modem Channels

Meshtastic uses the word "channels" to define two different configuration properties: [Messaging Channels](/docs/configuration/config/channels) & [LoRa Modem Channels](/docs/configuration/config/lora#channel-number)
...
- Leave your [MAX HOPS](/docs/configuration/config/lora#max-hops) set to 3 unless you're sure you need more (or less) to reach your destination node.
- Leave your [ROLE](/docs/configuration/config/device#role) set to `CLIENT` unless you're sure another role would suit the node's purpose.  For example, MQTT nodes do not need to be set to `ROUTER`.
- TEST your settings and hardware before you install in hard-to-reach locations.
- Connecting a node to the [public MQTT server](http://localhost:3000/docs/configuration/moduleconfig/mqtt#connect-to-the-default-public-server) may publish the locations of all nodes in your mesh to the internet.  This will also add every globally connected node to your node database and potentially flood your mesh with all types of packets.