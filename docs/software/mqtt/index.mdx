...
You can find the settings available for MQTT [here](/docs/configuration/moduleconfig/mqtt).
...
If no specific [root topic](/docs/configuration/moduleconfig/mqtt#root-topic) is configured, the default root topic will be `msh/`.
...
If [encryption_enabled](/docs/configuration/moduleconfig/mqtt#encryption-enabled) is set to true, the payload of the MeshPacket will remain encrypted with the key for the specified channel.

#### JSON topic
If [JSON is enabled](/docs/configuration/moduleconfig/mqtt/#json-enabled), packets from the following [port numbers](/docs/development/firmware/portnum) are serialized to JSON: `TEXT_MESSAGE_APP`, `TELEMETRY_APP`, `NODEINFO_APP`, `POSITION_APP`, `WAYPOINT_APP`, and `NEIGHBORINFO_APP`. These are then forwarded to the topic:
...
Check out [MQTT Settings](/docs/configuration/moduleconfig/mqtt) for full information. For quick start instructions, read on.
